package uz.bakhodirov.census.services.file;
...
import uz.bakhodirov.census.repository.file.UploadsRepository;
...
public class FileStorageService {
    public static final String UNICORN_UPLOADS_B_4_LIB = "D:/uploads/project/";
    ...
    public UploadsDto loadResource(@NonNull String fileName) throws NoSuchFileException {
        ...
        UploadsDto build = UploadsDto.builder().resource(resource).originalName(uploads.get().getOriginalName()).newName(uploads.get().getGeneratedName()).contentType(uploads.get().getContentType()).size(uploads.get().getSize()).build();
    return build;
    }
